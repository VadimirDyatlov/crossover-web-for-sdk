...
// TODO: Моки требуют доработки
export const merchantHandler = http.get('/api/crossover/v1/merchant', () => {
  return HttpResponse.json(mockMerchant);
});

export const productListHandlers = [
  http.get('/api/crossover/v1/product/list/1', () => {
    return HttpResponse.json({
      pagination,
      products: mockProducts1,
    });
  }),
  http.get('/api/crossover/v1/product/list/2', () => {
    return HttpResponse.json({
      pagination,
      products: mockProducts2,
    });
  }),
  http.get('/api/crossover/v1/product/list/3', () => {
    return HttpResponse.json({
      pagination,
      products: mockProducts3,
    });
  }),
  http.get('/api/crossover/v1/product/list/4', () => {
    return HttpResponse.json({
      pagination,
...
export const orderListHandler = http.get('/api/crossover/v1/order/list', () => {
  return HttpResponse.json({
